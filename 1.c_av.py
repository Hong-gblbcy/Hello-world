...
class RTSPStreamProcessor:
        ...
        while self.running:
            try:
                ...
                for frame in container.decode(stream):
                    ...
                    # 添加到队列
                    timestamp = time.time()
                    self.frame_queues[url].put((img, timestamp))
    ...
    def _frame_processing(self, frame, url):
        ...
        # 添加时间戳
        # 这里使用NumPy直接操作图像数据，而不是PIL的绘图功能
        gray_array = np.array(gray_image)
        
        # 返回处理结果和源URL
        return gray_array, url
    ...
    def _handle_processed_frame(self, processed_frame, url):
        ...
        Args:
            processed_frame: 处理过的帧 (NumPy数组)
            url: 来源RTSP URL
        """
        # 示例：将NumPy数组转换为PIL图像并保存
        # 实际应用中，你可能会将帧保存到磁盘、通过网络发送或进行其他操作
        try:
            # 创建PIL图像
            ...
            # 解析URL以获取干净的文件名
            # parsed_url = urlparse(url)
            # filename = f"output/{parsed_url.netloc}{parsed_url.path.replace('/', '_')}_{time.time()}.jpg"
